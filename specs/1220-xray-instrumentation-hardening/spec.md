...
3. **Given** active SSE connection drops, **When** reconnection occurs, **Then** fresh trace ID with stored previousTraceId for correlation.
...
- **FR-004**: ADOT collector config MUST export to X-Ray using existing sampling rules (100% dev/preprod, graduated prod).
...
- **FR-008**: All frontend fetch() calls (REST and SSE) MUST include X-Amzn-Trace-Id header via SSEConnection headers option and API client.
- **FR-009**: SSE reconnection MUST generate new trace ID and include stored previousTraceId for correlation.
...
- **ConditionalCheck/Count Metric**: New metric for DynamoDB optimistic locking contention. Expected at low rates, pathological at high rates.
...
- **SC-006**: ConditionalCheck/Count baseline established within 7 days for alarm calibration.
