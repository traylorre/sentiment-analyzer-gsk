...
Companion artifacts: a `CLEANUP-BOARD.html` kanban card recording root cause "unsafe autofix vs behavior-neutrality" and proposed fix "StrEnum migration feature with serialization test sweep". The tech-debt registry and constitution §9 that previously governed this were deleted 2026-07-31. Plus FR-014 lock-test module in `tests/unit/` asserting `str(member)` == `"ClassName.MEMBER"` and `.value` == wire string for all 7 (verified semantics on repo Python 3.13.0; none of the 7 defines `__str__`/`__format__`).
