...
- [X] T016 [P] [US3] Record the 7 UP042 suppressions as tech debt. **Target changed 2026-07-31**: the tech-debt registry was deleted and the constitution section that mandated it (§9) was removed by feature `001-constitution-prune`; debt is now a kanban card on `CLEANUP-BOARD.html`. Satisfied by the card "7 UP042 noqa riders on (str, Enum) classes (was TD-023)", lane `nice`, which carries the location, root cause, guard test and proposed StrEnum migration. No further action.
