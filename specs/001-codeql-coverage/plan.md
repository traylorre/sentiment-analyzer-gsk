...
|---|---|---|
| §3 Security & Access Control ("include SAST and secret scanning in CI") | This feature widens SAST coverage from one language to two and from about 0 to about 47,300 lines of previously unanalyzed JavaScript and TypeScript. | **PASS**, strengthened |
...
| §8 Git Workflow & CI/CD (pre-push, GPG signing, feature branch) | All commits GPG-signed, work on `001-codeql-coverage`. **`make validate` is NOT the pre-push gate for this feature and no task depends on it exiting 0**: `scripts/check-banned-terms.sh` exits 1 on pre-existing matches from other features' directories, so the target cannot pass on this tree for reasons that predate this feature. The substituted gate is defined at T005 and re-run at T030: the banned-terms scanner must report ZERO matches under `specs/001-codeql-coverage/`, which is the only banned-terms result this feature owns. | **PASS** against the substituted gate, stated rather than assumed |
...
├── spec.md                         # Input. Two appendices: "Adversarial Review #1" and the
│                                   #   2026-07-30 "Clarifications" session (Q1 to Q5, 2 DEFERRALS).
│                                   #   901 lines measured at this commit, but the count moves with
│                                   #   every spec edit, so read the appendix names, not the number.
├── plan.md                         # This file
...
- **D-4. The config is shared across both legs, and FR-006 resolves in two parts.** The
  ...
  scope. The resulting asymmetry against excluded Python test code is already argued in full at
  Clarification Q4, so the implementer TRANSCRIBES that argument into the evidence log rather than
  re-deriving it. Re-deriving invites a second argument that disagrees with the first, and a spec
  that argues with itself is the defect FR-007a exists to prevent.
...
- **D-7. FR-022 warning comment.** Modeled on the existing `playwright-e2e` warning at
  ...
  **Refutation of Adversarial Review #2 finding 18, recorded here so the citation above is not
  "fixed" into a wrong one.** Finding 18 claims D-7's `390-394` is off by one at both ends and
  should start at 389 and end at 393. Re-executed against the working tree: line 389 is a bare `#`,
  the warning paragraph is exactly lines 390 to 394, line 395 is the `# ====` rule and line 396
  opens `playwright-e2e:`. D-7's citation is correct as authored and finding 18 is WRONG. Finding
  18's own text survives at the foot of this file as append-only review history; do not apply it.
  Its wrong line pair is deliberately NOT written out here in full, because this block carries a
  guard that greps for exactly that string.
...
| 17 | LOW | **Line-count inconsistency for the newly analyzed surface.** `plan.md:44` and F10 say about 47,300; `plan.md:55` says "about 0 to about 48,000"; `spec.md:618` and `spec.md:647` say "about 48,000". Measured is 47,298. | FIXED at T003 and T004. Every rendering in the two files' bodies now reads about 47,300. The quoted strings above are retained verbatim as the finding text. |
| 18 | LOW | **`plan.md:136` cites the Playwright warning at `pr-checks.yml:390-394`.** The warning block is lines 389-393; 394 is the `# ====` rule and 395 opens `playwright-e2e:`. Off by one at both ends. | REFUTED at T004 and re-verified by execution at implementation time. Line 389 is a bare `#`, the warning paragraph is exactly 390 to 394, 395 is the `# ====` rule, 396 opens `playwright-e2e:`. D-7 is correct as authored. The refutation is recorded next to D-7 so a later reader does not "fix" a correct citation. |
| 19 | LOW | **`spec.md:26` F3 cites `codeql-config.yml` "lines 19 to 30".** The file is 29 lines. Q4 at `spec.md:792` cites 23 to 29 correctly, so the spec contains both the wrong and the right citation for the same block. | FIXED at T003. F3 now cites lines 19 to 20 and 23 to 29, matching Q4. |
...
| 21 | LOW | **`spec.md:5` Status is still `Draft`** after an adversarial review, a five-question clarification session, a plan, a quickstart, and a second adversarial review. | FIXED at T003. Status set to `Ready for Implementation`. Note the premise is weaker than the finding implies: measured across 216 specs in this repository, 191 sit at `Draft`, including every already-merged feature of this campaign, so there is no established convention for a planned feature. The nearest existing value was chosen. |
...
**Closed at implementation time (T003, T004, T004a).** Findings 12, 13, 14, 17, 19 and 21 are
FIXED; finding 18 is REFUTED. The Fix column above carries each disposition.

**This findings table is append-only in its Finding column, and that has a consequence for
T004's own pass condition 1.** That condition greps `plan.md` FILE-WIDE for `about 48,000` and
`is argued or carded` and requires `rc=1`. Both strings survive here, inside findings 17 and 14,
as the verbatim quotations of the defects being recorded. The condition is therefore unpassable
on a correctly fixed tree unless review history is falsified, which is the exact trap T004 pass
condition 2 was rewritten to avoid and which pass condition 1 was left open to. **The runnable
form is the body-scoped one**, which excludes this appendix:

```bash
sed -n "1,$(( $(grep -n '^## Adversarial Review #2' plan.md | cut -d: -f1) - 1 ))p" plan.md \
  | grep -n 'about 48,000\|is argued or carded'; echo "rc=$?"     # rc=1 is clean
```
