...
- [ ] T008 Add rate limit and concurrent lock checks to `create_experiment()` and `start_experiment()`:
...
- [ ] T009 Update `stop_experiment()` in chaos.py:
...
- [ ] T010 Add `_handle_auto_restore(experiment_id)` function to handler.py (or chaos.py):
...
- [ ] T011 Add EventBridge Scheduler IAM role to `infrastructure/terraform/modules/chaos/main.tf`:
...
- [ ] T012 Add Scheduler + metrics permissions to Dashboard Lambda IAM policy in `infrastructure/terraform/modules/iam/main.tf`:
...
- [ ] T013 Add CloudWatch alarm for ChaosIAMPolicyAttachment to `infrastructure/terraform/modules/chaos/main.tf`:
...
- [ ] T014 Add `SCHEDULER_ROLE_ARN` and `DASHBOARD_LAMBDA_ARN` environment variables to Dashboard Lambda in `infrastructure/terraform/main.tf`:
  - `SCHEDULER_ROLE_ARN = try(module.chaos.chaos_scheduler_role_arn, "")`
  - `DASHBOARD_LAMBDA_ARN = module.dashboard_lambda.function_arn`

## Phase 4: Tests

- [ ] T015 Add unit tests `tests/unit/test_chaos_security.py`:
...
- [ ] T016 Add E2E tests `tests/e2e/test_chaos_lockdown_preprod.py` with marker `@pytest.mark.preprod`:
...
- [ ] T017 Update existing chaos tests (`tests/unit/test_chaos_fis.py`): Add mock for session validation and environment check. Ensure all existing tests still pass by:
...
- [ ] T018 Run backend unit tests: `python -m pytest tests/unit/ -x -q --timeout=120 -p no:playwright`
- [ ] T019 Run `make validate` (lint + security)
- [ ] T020 **[MANUAL]** After deploy: curl preprod chaos endpoints to verify 404 (SC-002)
