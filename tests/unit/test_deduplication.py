...
For Developers:
    - source_id format: "{source}#{hash16}" where source is tiingo or finnhub
...
class TestGenerateSourceId:
    ...
    def test_generate_from_url(self):
        ...
        assert source_id.startswith("tiingo#")
        assert len(source_id) == len("tiingo#") + 16
...
class TestIsDuplicate:
    ...
    def test_is_duplicate_true(self):
        ...
        existing = {"tiingo#abc123", "tiingo#def456"}

        assert is_duplicate("tiingo#abc123", existing) is True

    def test_is_duplicate_false(self):
        """Test new item returns False."""
        existing = {"tiingo#abc123", "tiingo#def456"}

        assert is_duplicate("tiingo#new789", existing) is False

    def test_empty_existing_set(self):
        """Test with empty existing set."""
        existing: set[str] = set()

        assert is_duplicate("tiingo#abc123", existing) is False
...
class TestBatchDeduplicate:
    ...
    def test_adds_source_id_to_new_articles(self):
        ...
        assert new[0]["source_id"].startswith("tiingo#")
    ...
    def test_returns_combined_ids(self):
        ...
        assert "tiingo#existing1" in all_ids
