...
class TestOHLCHappyPath:
    ...
    def test_ohlc_valid_ticker_default_params(
        ...
        cache_source = get_response_header(response, "X-Cache-Source")
        assert cache_source != "", "X-Cache-Source header missing"
        ...
        cache_age = get_response_header(response, "X-Cache-Age")
        assert cache_age != "", "X-Cache-Age header missing"
