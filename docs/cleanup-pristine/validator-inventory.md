...
| ruff-check (lint incl. security `S` rules) | Makefile:63-66; pre-commit:56-57; pyproject:74-91 | pr-checks.yml:61-65 | Yes, blocking | None |
...
One CI job, `pre-commit run --all-files` (the commented-out block at .pre-commit-config.yaml:179-184), pulls detect-secrets, trivy, checkov, and staged mypy under server-side enforcement in a single step. Today CI enforcement is limited to jobs that invoke tools directly: ruff, gitleaks, pytest+coverage, Playwright, and non-blocking pip-audit.
